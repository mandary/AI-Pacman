...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        oldb = self.beliefs.copy()
        self.beliefs = util.Counter()
        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newp, pr in newPosDist.items():
                self.beliefs[newp] += pr * oldb[p]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None: # ghost eaten
            ...
            if weights.totalCount() is not 0:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        ghosts = list(itertools.permutations(self.legalPositions, r = self.numGhosts))
        random.shuffle(ghosts)
        for i in range(self.numParticles):
            self.particles.append(tuple(ghosts[i % len(ghosts)]))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            self.numGhosts = self.numGhosts
            for i in range(self.numGhosts):
                newParticle[i] = util.sampleFromCounter(getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), i, self.ghostAgents[i]))
            "*** END YOUR CODE HERE ***"
